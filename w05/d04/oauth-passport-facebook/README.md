...
- My Apps > Add a New App
  - Enter an app display name and provide an email address
  - Create the App
  - Pass the Facebook Captcha
- Click "Set Up" under "Facebook Login"
  - Click "Settings" under "Facebook Login"
  - under "OAuth redirect URIs", enter "http://localhost:3000/auth/facebook/callback"
  - click "Save Changes"
...
Get the App ID and App Secret from the Facebook App's Dashboard.

In a `.env` file, save the App ID as `FACEBOOK_APP_ID` and save the App Secret as `FACEBOOK_APP_SECRET`.

```
FACEBOOK_APP_ID=123456789
FACEBOOK_APP_SECRET=123456789123456789123456789
```
...
As we've already seen when we've used `passport-local`, passport use strategies.  `passport-facebook` uses the same process!  In the file `config/setup-facebook-auth.js` we need to add a lot of code:
...
function setupFacebookAuthStrategy(passport) {
  ...
  passport.use('facebook', new FacebookStrategy({
    ...
    // profile contains info returned from Facebook
    // console.log(profile);
...
<html>
<head>
  <title>My OAuth-Enabled App</title>
</head>
<body>
  <h1>Log In with Facebook</h1>
  <div>
    <% if (user != undefined) { %>
      <h2>Below is the data from Facebook</h2>
      <pre>
        <%= user.fb %>
      </pre>
